  ...
  useEffect(() => {
    ...
    if (latLngs.length === 0) {
      ...
      if (isManualLatLng) {
        ...
        // Handle coordinates that might be raw or already mapped
        const x = plot.x < 1000 ? plot.x * 10 + 495000 : plot.x;
        const y = plot.y < 1000 ? plot.y * 10 + 2766000 : plot.y;
        [lat, lng] = convertToLatLng(x, y);
      }
      const offset = 0.0005;
      latLngs = [
    ...
    // Invalidate size once before animation to handle tab switches
    map.invalidateSize();
